...
export default function DeepSeekJanitorAIBody() {
  const howToSchema = {
    ...
    "@type": "HowTo",
    "name": "How to Configure DeepSeek on Janitor AI",
    "step": [
      {
        "@type": "HowToStep",
        "name": "Generate DeepSeek API Key",
        "text": "Register on the DeepSeek platform and generate a pre-paid API key."
      },
      {
        "@type": "HowToStep",
        "name": "Pre-pay Credits",
        "text": "Pre-pay account credit with at least a $2 minimum balance."
      },
      {
        "@type": "HowToStep",
        "name": "Set base URL and proxy settings",
        "text": "Paste base URL https://api.deepseek.com/v1 and your API Key inside Janitor AI settings."
      }
    ]
  };
  ...
  return (
    <>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', fontSize: '14px' }}>
          {[
            ...
            { label: 'Model Name (Chat Direct)', value: 'deepseek-v4-flash', desc: 'Matches the post-July 2026 low-latency, non-thinking option' },
            { label: 'Model Name (Thinking Direct)', value: 'deepseek-v4-pro', desc: 'Matches the post-July 2026 reasoning model option' },
            { label: 'Model Name (OpenRouter Chat)', value: 'deepseek/deepseek-v4-flash', desc: 'Routing string for OpenRouter non-thinking' },
      ...
      <ul style={{ margin: '24px 0', paddingLeft: '24px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
        ...
        <li>
          <strong style={{ color: 'var(--ink-primary)' }}>Model Name:</strong> <code style={{ fontFamily: 'var(--font-mono)', fontSize: '0.875em', background: 'var(--bg-warm-gray)', padding: '2px 8px', borderRadius: '4px' }}>deepseek-v4-flash</code> (for Direct Chat) or <code style={{ fontFamily: 'var(--font-mono)', fontSize: '0.875em', background: 'var(--bg-warm-gray)', padding: '2px 8px', borderRadius: '4px' }}>deepseek-v4-pro</code> (for Direct Thinking / reasoning), or their OpenRouter equivalents like <code style={{ fontFamily: 'var(--font-mono)', fontSize: '0.875em', background: 'var(--bg-warm-gray)', padding: '2px 8px', borderRadius: '4px' }}>deepseek/deepseek-v4-flash</code>.
        </li>
      </ul>
  ...
  "max_tokens": 4096
      ...
      <p>
        DeepSeek&apos;s post-July 2026 lineup features <strong>DeepSeek V4 Pro</strong> (the reasoning/thinking mode option, replacing the old <code>deepseek-reasoner</code>), which uses chain-of-thought processing to &quot;think&quot; before it speaks. While incredible for coding or math, <strong>avoid using thinking-mode models like V4 Pro for standard roleplay in Janitor AI.</strong>
      </p>
      <p>
        Reasoning models output raw <code>&lt;think&gt;...&lt;/think&gt;</code> tags containing their internal monologue. Janitor AI’s frontend currently struggles to parse or hide these tags cleanly, which immediately breaks immersion during a character interaction. For seamless roleplay and character interactions, always stick to <strong>DeepSeek V4 Flash</strong> (<code>deepseek-v4-flash</code>, which replaces the legacy <code>deepseek-chat</code>). It remains lightning fast, highly responsive, completely uncensored, and parses perfectly in Janitor’s interface.
      </p>
      <p>
      ...
      <ul style={{ margin: '24px 0', paddingLeft: '24px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
        <li><strong style={{ color: 'var(--ink-primary)' }}>Temperature (1.0 - 1.3):</strong> Start around 1.0. For more creative prose and less rigid, repetitive patterns in storytelling, safely bump it up toward 1.3.</li>
        <li><strong style={{ color: 'var(--ink-primary)' }}>Top-P (1.0):</strong> Controls sampling diversity. Setting this to 1.0 is highly recommended for the best balance of creativity and coherence.</li>
        <li><strong style={{ color: 'var(--ink-primary)' }}>Max Tokens (2048 - 4096):</strong> Set this high enough (such as 4096) so long character responses do not abruptly cut off mid-sentence.</li>
      </ul>
...
router_settings:
  ...
  fallbacks: [{"deepseek-v4-flash": ["ollama/llama3"]}]`}</code>
        </pre>
      </div>
      <p>
      ...
      <ul style={{ margin: '24px 0', paddingLeft: '24px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
        ...
        <li><strong style={{ color: 'var(--ink-primary)' }}>&quot;Model Not Found&quot; Error:</strong> You typo&apos;d the Model ID. It must be exactly <code style={{ fontFamily: 'var(--font-mono)', fontSize: '0.875em', background: 'var(--bg-warm-gray)', padding: '2px 8px', borderRadius: '4px' }}>deepseek-v4-flash</code>.</li>
