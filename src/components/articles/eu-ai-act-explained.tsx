...
export default function EuAiActExplainedBody() {
  return (
    <>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '14px', color: 'var(--ink-secondary)', background: 'var(--bg-white)', border: '1px solid var(--border-light)', borderRadius: 'var(--radius-md)' }}>
          <thead>
            <tr style={{ background: 'var(--bg-cream)', borderBottom: '1px solid var(--border-light)' }}>
              <th style={{ padding: '12px 16px', textAlign: 'left', fontWeight: 700, color: 'var(--ink-primary)' }}>Risk Category</th>
              <th style={{ padding: '12px 16px', textAlign: 'left', fontWeight: 700, color: 'var(--ink-primary)' }}>Concrete Examples</th>
              <th style={{ padding: '12px 16px', textAlign: 'left', fontWeight: 700, color: 'var(--ink-primary)' }}>Key Regulatory Obligations</th>
              <th style={{ padding: '12px 16px', textAlign: 'left', fontWeight: 700, color: 'var(--ink-primary)' }}>Enforcement Timeline</th>
            </tr>
          </thead>
          <tbody>
            {[
              {
                ...
                examples: 'Social scoring, real-time public biometric scanning, subliminal behavior manipulation.',
                obligations: 'Complete ban on development, importation, or use inside the EU.',
                timeline: 'Banned as of February 2, 2025'
              },
              {
                tier: '⚠️ High Risk',
                examples: 'CV screening, credit scoring, medical diagnostics, critical utilities, law enforcement profiling.',
                obligations: 'Third-party conformity audit, EU database registration, mandatory logging, and human-in-the-loop overrides.',
                timeline: 'Fully active on August 2, 2026'
              },
              {
                tier: '🔍 Limited Risk',
                examples: 'Customer support chatbots, generative image/text tools, deepfakes, emotion recognition.',
                obligations: 'Explicit transparency: users must be clearly informed they are interacting with an AI system.',
                timeline: 'Fully active on August 2, 2025'
              },
              {
                tier: '✅ Minimal Risk',
                examples: 'Spam filters, inventory management, gaming NPCs, simple text formatting APIs.',
                obligations: 'No regulatory restrictions. Encouraged to follow voluntary codes of conduct.',
                timeline: 'No deadline / Free to deploy'
              }
            ].map((row, idx) => (
              <tr key={idx} style={{ borderBottom: idx === 3 ? 'none' : '1px solid var(--border-light)' }}>
                <td style={{ padding: '12px 16px', fontWeight: 700, color: 'var(--ink-primary)' }}>{row.tier}</td>
                <td style={{ padding: '12px 16px' }}>{row.examples}</td>
                <td style={{ padding: '12px 16px' }}>{row.obligations}</td>
                <td style={{ padding: '12px 16px', fontWeight: 600, color: 'var(--accent-orange)', borderLeft: '1px solid var(--border-light)' }}>{row.timeline}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      <p>
        When you run local inference and maintain your own <code style={{ fontFamily: 'var(--font-mono)', fontSize: '0.875em', background: 'var(--bg-warm-gray)', padding: '2px 8px', borderRadius: '4px' }}>pgvector</code> instance, you eliminate third-party data processing agreements. You control the data residency.
      </p>
